...
export function MasterGuideForm({ guide, triggerLabel, prefill }: Props) {
  ...
  const [sectionsJson, setSectionsJson] = useState(
    JSON.stringify(guide?.sections ?? prefill?.sections ?? [], null, 2)
  )
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (f.sections) setSectionsJson(JSON.stringify(f.sections, null, 2))
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const sec = parseJson<GuideSection[]>('Sektioner', sectionsJson, [])
    if ('error' in sec) { setError(sec.error); return }
    const cal = parseJson<GuideCalendarRule[]>('Kalender-regler', calendarRulesJson, [])
    if ('error' in cal) { setError(cal.error); return }
    if (!Array.isArray(sec)) { setError('Sektioner skal være et array.'); return }
